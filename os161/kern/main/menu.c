...
cmd_progthread(void *ptr, unsigned long nargs)
{
	...
	char** data = kmalloc(nargs*sizeof(char*));

//	int i;
//	for(i=0; i<nargs ; i++){
//		data[i] = (char*)(kmalloc((strlen(args[i])+1)*sizeof(char)));
//		copyinstr(args[i], data, 128, NULL);
//	}
...
cmd_df(int nargs, char **args)
{
	...
	else if (atoi(args[1]) < 1 || atoi(args[1]) > 12) {
		retval = EINVAL;
	}
	else if (strcmp(args[2], "off") != 0 && strcmp(args[2], "on") != 0) {
		retval = EINVAL;
	}
	...
	int numToShift = atoi(args[1]) - 1;

	// do the bit manipulation for dbflag
...
common_prog(int nargs, char **args)
{
	...
	struct thread* waitthread;

	result = thread_fork(args[0] /* thread name */,
			args /* thread arg */, nargs /* thread arg */,
			cmd_progthread, &waitthread);
	...
	int retval;
	return sys_waitpid(waitthread->pid, &status, 0, &retval);
