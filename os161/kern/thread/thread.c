...
thread_create(const char *name)
{
	...
	// them here.
	//////////////////////////Lab 2//////////////////////////////////
	//initialize threads with pid, parent, child array
	thread->pid = -1;
	thread->parent = -1;
//	int i;
...
//		thread->child[i] = 0;
//	}
	thread->wait = sem_create("wait", 0);
	thread->pid = pidcount;

	//setup process
	process->pid = thread->pid;
	process->parent = -1;
	process->haswaiting = 0;
	process->exitcode = -1;
	process->exited = 0;
	process->thread = thread;

	all_thread[thread->pid] = process;
...
thread_destroy(struct thread *thread)
{
	...
	if (thread->wait != NULL) sem_destroy(thread->wait);
	    ...
	    struct thread **ret)
{
	...
	//set newguy parent
	newguy->parent = curthread->pid;

//	//set parent thread's child array
//	curthread->child[newguy->pid] = 1;

	//store newguy
	all_thread[newguy->pid]->parent = curthread->pid;
	all_thread[newguy->pid]->thread = newguy;
	...
	//kprintf("in threadfork table:%d\n",all_thread[curthread->pid]->pid);
...
//	kprintf("thread_exit %d\n", curthread->pid);

//	pid_t parent_pid = all_thread[curthread->pid]->parent;
//	struct thread* parent_thread = all_thread[parent_pid]->thread;
//	parent_thread->child[curthread->pid] = 0;

	all_thread[curthread->pid]->exited = 1;

	V(all_thread[curthread->pid]->thread->wait);

//	V(lock);
//	kprintf("\nthread_exit lock sem count %d\n", lock->count);
	///////////////////////////////////////////////////////////
...
void
thread_wakeoneup(const void *addr)
{
	int i, result;
